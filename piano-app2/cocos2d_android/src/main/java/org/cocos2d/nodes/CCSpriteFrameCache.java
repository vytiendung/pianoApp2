...
//		  }
    ...
    */
    ...
    public Set<String> addSpriteFrames(String plist,Bitmap bitmap) {
        String texturePath = null;
        int i = plist.lastIndexOf('.');
        if (i > 0 && i <= plist.length() - 2)
		      texturePath = plist.substring(0, i) + ".png";
        CCTexture2D texture =
            CCTextureCache.sharedTextureCache().addImage(bitmap,texturePath,true);
        return addSpriteFrames(plist, texture);
    }
    
    public void removeSpriteFrames ( String plist , boolean releaseTexture ) {
    	 if ( releaseTexture ) {
	    	 String texturePath = null;
	         int i = plist.lastIndexOf('.');
	         if (i > 0 && i <= plist.length() - 2)
	 		      texturePath = plist.substring(0, i) + ".png";
	         CCTexture2D texture =
	             CCTextureCache.sharedTextureCache().getTexture(texturePath);
			 if ( texture != null) {
				 CCTextureCache.sharedTextureCache().removeTexture(texturePath);
     ...
     * The texture will be associated with the created sprite frames.
     */
    public void removeSpriteFrames(HashMap<String, Object> dictionary) {
    	/*
   	 Supported Zwoptex Formats:
   	 ZWTCoordinatesFormatOptionXMLLegacy = 0, // Flash Version
   	 ZWTCoordinatesFormatOptionXML1_0 = 1, // Desktop Version 0.0 - 0.4b
      ...
      if (!(format >= 0 && format <= 3)) {
    	...
    	for(Entry<String, Object> frameDictEntry : framesDict.entrySet()) {
			...
			HashMap<String, Object> frameDict = (HashMap<String, Object>)frameDictEntry.getValue();
    		// remove sprite frame
    		spriteFrames.remove(frameDictEntry.getKey());
    	}
    }
    //end
}
