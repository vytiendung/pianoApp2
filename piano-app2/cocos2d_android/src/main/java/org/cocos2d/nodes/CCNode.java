...
public class CCNode {
    ...
    */
    ...
    protected boolean canDraw(GL10 gl){
        float[] matrix = new float[16];
        ((GL11) gl).glGetFloatv(GL11.GL_MODELVIEW_MATRIX, matrix, 0);
        CGAffineTransform transform = new CGAffineTransform(matrix[0],matrix[1],matrix[4],matrix[5],matrix[12],matrix[13]);
        for ( int i = 0 ; i < 8 ; ++i ) {
            matrix[i] = 0.0f;
        }
        matrix[2] = contentSize_.width;
        matrix[5] = contentSize_.height;
        matrix[6] = contentSize_.width;
        matrix[7] = contentSize_.height;
        transform.transform(matrix,0,matrix,8,4);
        float minX = matrix[8] , maxX = matrix[8];
        float minY = matrix[9] , maxY = matrix[9];
        for (int i = 5; i < 8; ++i) {
            minX = Math.min(minX,matrix[2 * i]);
            minY = Math.min(minY,matrix[2 * i + 1]);
            maxX = Math.max(maxX,matrix[2 * i]);
            maxY = Math.max(maxY,matrix[2 * i + 1]);
        }
        final CGSize winSize = CCDirector.sharedDirector().winSizeRef();
        boolean ret = ( maxX > -winSize.width*0.5 && minX < winSize.width*0.5) &&
                (minY  < winSize.height*0.5 && maxY > -winSize.height*0.5);
        return true;
    }
    ...
    public CGAffineTransform nodeToParentTransform() {
        if (isTransformDirty_) {
            ...
            if (!isRelativeAnchorPoint_ && !CGPoint.equalToPoint(anchorPointInPixels_, zero)) {
                tx += anchorPointInPixels_.x;
                ty += anchorPointInPixels_.y;
