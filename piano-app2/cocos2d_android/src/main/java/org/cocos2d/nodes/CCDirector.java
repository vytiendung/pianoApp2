...
public class CCDirector implements GLSurfaceView.Renderer {
	...
	private ArrayList<FrameEventListener> frameEventListeners;
	private FrameEventListener addingListener;
	private FrameEventListener removingListener;
...
//                gl.glViewport(0, 0, (int)size.width, (int)size.height);
	...
	private static final boolean FAST_FPS_DISPLAY = false;
...
//		return;
		...
		waitForFPS();
		openGLView_.setNeedsSwap();
//        }
	}

	private void dispatchFrameEvent() {
		if (frameEventListeners != null) {
			for (FrameEventListener listener : frameEventListeners) {
				if (listener != removingListener) {
					listener.onEnterFrame(dt);
				}
			}
			if (removingListener != null) {
				performRemoveFrameEventListener(removingListener);
				removingListener = null;
			}
		}
		if (addingListener != null) {
			performAddFrameEventListener(addingListener);
			addingListener = null;
		}
	}

	private void performAddFrameEventListener(FrameEventListener listener) {
		if (frameEventListeners == null) {
			frameEventListeners = new ArrayList<>();
		}
		if (frameEventListeners.indexOf(listener) < 0) {
			frameEventListeners.add(listener);
		}
	}

	private void performRemoveFrameEventListener(FrameEventListener listener) {
		if (frameEventListeners != null) {
			frameEventListeners.remove(listener);
		}
	}

	public void addFrameEventListener(FrameEventListener listener) {
		addingListener = listener;
	}
...
//		stopAnimation();
		...
		// OpenGL view
		openGLView_ = null; //TODO: Giang fix mem leak
//    		_sharedDirector = null;
//            if (FPSLabel_ != null)
//                FPSLabel_ = null;
//    	}
	...
	private void showFPS(GL10 gl) {
		...
		if (FAST_FPS_DISPLAY) {
			...
			if (accumDt_ > ccConfig.CC_DIRECTOR_FPS_INTERVAL) {
				...
				fpsBuilder.append(CCFormatter.format("-%.2fM",GLResourceHelper.sharedHelper().getTextMemoryUsage()/ ( 1024.0f* 1024.0f)));
				//end
...
//            String str = CCFormatter.format("%.2f", frameRate_);
			String str = CCFormatter.format("%.2f-%.2fM", frameRate_,GLResourceHelper.sharedHelper().getTextMemoryUsage()/(1024.0f*1024.0f));
			//end
...
//            texture.initWithText(str, CGSize.make(100, 30), TextAlignment.LEFT, "DroidSans", 24);
			texture.initWithText(str, CGSize.make(150, 30), TextAlignment.LEFT, "DroidSans", 24);
			//end
	...
	public CCNode getTopScene() {
		int size = CCScenesStack_.size();
		if ( size > 0 ) {
			return CCScenesStack_.get(size-1);
		}
		return null;
	}

	public void sendCleanupToCCScene ( boolean cleanup ) {
		sendCleanupToCCScene_ = cleanup;
	}
	//end
}
