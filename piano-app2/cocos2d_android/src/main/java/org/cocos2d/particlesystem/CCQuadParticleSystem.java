 ...
 * 
 * Special features and Limitations: - Particle size can be any float number. -
 * The system can be scaled - The particles can be rotated - On 1st and 2nd gen
 * iPhones: It is only a bit slower that CCPointParticleSystem - On 3rd gen
 * iPhone and iPads: It is MUCH faster than CCPointParticleSystem - It consumes
 * more RAM and more GPU memory than CCPointParticleSystem - It supports
 * subrects
 * 
 * @since v0.8
 */
public class CCQuadParticleSystem extends CCParticleSystem implements Resource {
	...
	private static class QuadParticleLoader implements
			GLResourceHelper.GLResourceLoader {
		...
		public void load(Resource res) {
			...
			gl.glBufferData(GL11.GL_ARRAY_BUFFER, thisp.texCoords.capacity(),
					thisp.texCoords, GL11.GL_DYNAMIC_DRAW);

			// for vertices
			gl.glBindBuffer(GL11.GL_ARRAY_BUFFER, thisp.quadsIDs[1]);
			thisp.vertices.position(0);
			gl.glBufferData(GL11.GL_ARRAY_BUFFER, thisp.vertices.capacity(),
					thisp.vertices, GL11.GL_DYNAMIC_DRAW);

			// for colors
			gl.glBindBuffer(GL11.GL_ARRAY_BUFFER, thisp.quadsIDs[2]);
			thisp.colors.position(0);
			gl.glBufferData(GL11.GL_ARRAY_BUFFER, thisp.colors.capacity(),
					thisp.colors, GL11.GL_DYNAMIC_DRAW);

			// for indices
			// ByteBuffer indices =
			// BufferUtils.newUnsafeByteBuffer(thisp.totalParticles * 6 * 2);
			// ByteBuffer indices =
			// BufferUtils.newByteBuffer(thisp.totalParticles * 6 * 2);
			ByteBuffer indices = null;
			if (BufferUtils.USING_NATIVE) {
				indices = BufferUtils
						.newUnsafeByteBuffer(thisp.totalParticles * 6 * 2);
			} else {
				indices = BufferUtils
						.newByteBuffer(thisp.totalParticles * 6 * 2);
			}
			thisp.initIndices(indices);
			gl.glBindBuffer(GL11.GL_ELEMENT_ARRAY_BUFFER, thisp.quadsIDs[3]);
			gl.glBufferData(GL11.GL_ELEMENT_ARRAY_BUFFER, indices.capacity(),
	...
	protected void init() {
		...
		if (BufferUtils.USING_NATIVE) {
			texCoords = BufferUtils
					.newUnsafeByteBuffer(4 * 2 * totalParticles * 4);
			vertices = BufferUtils
					.newUnsafeByteBuffer(4 * 2 * totalParticles * 4);
			colors = BufferUtils
					.newUnsafeByteBuffer(4 * 4 * totalParticles * 4);
		} else {
			texCoords = BufferUtils.newByteBuffer(4 * 2 * totalParticles * 4);
			vertices = BufferUtils.newByteBuffer(4 * 2 * totalParticles * 4);
			colors = BufferUtils.newByteBuffer(4 * 4 * totalParticles * 4);
			// end
		}
	...
	public void finalize() throws Throwable {
		if (quadsIDs != null) {
			GLResourceHelper.sharedHelper().perform(
					new GLResourceHelper.GLResorceTask() {
	...
	public void initTexCoordsWithRect(CGRect rect) {
		initTexCoordsWithRect(rect.origin.x, rect.origin.y, rect.size.width,
				rect.size.height);
	}

	public void initTexCoordsWithRect(float rectX, float rectY, float rectW,
			float rectH) {
		...
		// Important. Texture in cocos2d are inverted, so the Y component should
		// be inverted
		float tmp = topRightY;
	...
	public void setTexture(CCTexture2D tex) {
		this.setTexture(tex,
				CGRect.make(0, 0, tex.pixelsWide(), tex.pixelsHigh()));
	}

	/**
	 * Sets a new CCSpriteFrame as particle. WARNING: this method is
	 * experimental. Use setTexture:withRect instead.
	 * 
	 * @since v0.99.4
	 */
	public void setDisplayFrame(CCSpriteFrame spriteFrame) {
		assert CGPoint.equalToPoint(spriteFrame.getOffsetRef(),
				CGPoint.getZero()) : "QuadParticle only supports SpriteFrames with no offsets";
	...
	public void postStep() {
		...
		gl.glBufferSubData(GL11.GL_ARRAY_BUFFER, 0, texCoords.capacity(),
				texCoords);

		// for vertices
		gl.glBindBuffer(GL11.GL_ARRAY_BUFFER, quadsIDs[1]);
		vertices.position(0);
		gl.glBufferSubData(GL11.GL_ARRAY_BUFFER, 0, vertices.capacity(),
				vertices);
	...
	public void draw(GL10 gle) {
		...
		if (blendFunc.src != ccConfig.CC_BLEND_SRC
		...
		/*
		 * if( particleIdx != particleCount ) { String str =
		 * String.format("pd:%d, pc:%d", particleIdx, particleCount);
		 * ccMacros.CCLOG("CCQuadParticleSystem", str); }
		 */

		// Log.e("ParticleSystem", "particleIdx is " +
		// String.valueOf(particleIdx));

		gl.glBindBuffer(GL11.GL_ELEMENT_ARRAY_BUFFER, quadsIDs[3]);
		gl.glDrawElements(GL11.GL_TRIANGLES, particleIdx * 6,
