 ...
 * runtime Quads can be re-ordered in runtime The TextureAtlas capacity can be
 * increased or decreased in runtime OpenGL component: V3F, C4B, T2F. The quads
 * are rendered using an OpenGL ES VBO. To render the quads using an interleaved
 * vertex array list, you should modify the ccConfig.h file
 */
public class CCTextureAtlas {
	...
	/**
	 * creates a TextureAtlas with an filename and with an initial capacity for
	 * Quads. The TextureAtlas capacity can be increased in runtime.
	 */
	...
	/**
	 * initializes a TextureAtlas with a filename and with a certain capacity
	 * for Quads. The TextureAtlas capacity can be increased in runtime.
	 * 
	 ...
	 * and with an initial capacity for n Quads. The TextureAtlas capacity can
	 * be increased in runtime.
	 */
	...
	/**
	 * initializes a TextureAtlas with a previously initialized Texture2D
	 * object, and with an initial capacity for Quads. The TextureAtlas capacity
	 * can be increased in runtime.
	 * 
	 * WARNING: Do not reinitialize the TextureAtlas because it will leak memory
	 * (issue #706)
	 */
	public CCTextureAtlas(CCTexture2D tex, int n) {
		...
		ByteBuffer tbb = ByteBuffer
				.allocateDirect(ccQuad2.size * capacity_ * 4);
		tbb.order(ByteOrder.nativeOrder());
		textureCoordinates = tbb.asFloatBuffer();

		ByteBuffer vbb = ByteBuffer
				.allocateDirect(ccQuad3.size * capacity_ * 4);
	...
	public String toString() {
		return new CCFormatter().format("<%s = %08X | getTotalQuads = %i>",
				CCTextureAtlas.class, this, totalQuads_);
	}

	public void initColorArray() {
		if (!withColorArray_) {
			// default color: 255,255,255,255
			// modify by zt: A Texture in TextureAtlas need four colors for four
			// vertices
			ByteBuffer cbb = ByteBuffer.allocateDirect(4 * capacity_
					* ccColor4B.size * 4);
	...
	/**
	 * updates a Quad (texture, vertex and color) at a certain index index must
	 * be between 0 and the atlas capacity - 1
	 * 
	 * @since v0.8
	 */
	public void updateQuad(FloatBuffer texCordBuffer, FloatBuffer vertexBuffer,
			int index) {
	...
	public void updateQuad(FloatBuffer texCordBuffer, float[] vertexData,
			int index) {
		assert (index >= 0 && index < capacity_) : "update quad with texture_: Invalid index";
	...
	/**
	 * Inserts a Quad (texture, vertex and color) at a certain index index must
	 * be between 0 and the atlas capacity - 1
	 * 
	 * @since v0.8
	 */
	public void insertQuad(FloatBuffer texCordBuffer, FloatBuffer vertexBuffer,
			int index) {
		...
		if (remaining > 0) {
			// tex coordinates
			arraycopyTexture(textureCoordinates, index, textureCoordinates,
					index + 1, remaining);

			// vertexCoordinates_
			arraycopyVertex(vertexCoordinates, index, vertexCoordinates,
					index + 1, remaining);
	 ...
	 * new index This operation is faster than removing and inserting in a quad
	...
	/**
	 * removes a quad at a given index number. The capacity remains the same,
	 * but the total number of quads to be drawn is reduced in 1
	 * 
	 * @since v0.7.2
	 */
	public void removeQuad(int index) {
		...
		if (remaining > 0) {
			// tex coordinates
			arraycopyTexture(textureCoordinates, index + 1, textureCoordinates,
					index, remaining);

			// vertexCoordinates_
			arraycopyVertex(vertexCoordinates, index + 1, vertexCoordinates,
					index, remaining);
	...
	/**
	 * removes all Quads. The TextureAtlas capacity remains untouched. No memory
	 * is freed. The total number of quads to be drawn will be 0
	 * 
	 * @since v0.7.2
	 */
	...
	/**
	 * resize the capacity of the Texture Atlas. The new capacity can be lower
	 * or higher than the current one It returns YES if the resize was
	 * successful. If it fails to resize the capacity it will return NO with a
	 * new capacity of 0.
	 */
	public void resizeCapacity(int newCapacity) {

		final int MAX_POSSIBLE_QUADS_COUNT = Short.MAX_VALUE / 6; // 6 short
																	// indices
																	// per quad
		...
		ByteBuffer tbb = ByteBuffer.allocateDirect(ccQuad2.size * newCapacity
				* 4);
		tbb.order(ByteOrder.nativeOrder());
		...
		ByteBuffer vbb = ByteBuffer.allocateDirect(ccQuad3.size * newCapacity
				* 4);
		vbb.order(ByteOrder.nativeOrder());
		...
		if (withColorArray_) {
			ByteBuffer cbb = ByteBuffer.allocateDirect(4 * ccColor4B.size
					* newCapacity * 4);
			cbb.order(ByteOrder.nativeOrder());
	...
	public void draw(GL10 gl, int n) {
		...
		if (ccConfig.CC_TEXTURE_ATLAS_USE_TRIANGLE_STRIP) {
			gl.glDrawElements(GL10.GL_TRIANGLE_STRIP, n * 6,
					GL10.GL_UNSIGNED_SHORT, indices);
		} else {
			gl.glDrawElements(GL10.GL_TRIANGLES, n * 6, GL10.GL_UNSIGNED_SHORT,
					indices);
		}
	...
	private ccColor4B[] getColor(FloatBuffer src, int index) {
		...
		for (int j = 0; j < 4; ++j) {
			color[j].r = (int) (255 * src.get(index * ccColor4B.size * 4 + 4
					* j + 0));
			color[j].g = (int) (255 * src.get(index * ccColor4B.size * 4 + 4
					* j + 1));
			color[j].b = (int) (255 * src.get(index * ccColor4B.size * 4 + 4
					* j + 2));
			color[j].a = (int) (255 * src.get(index * ccColor4B.size * 4 + 4
					* j + 3));
		}
	...
	private void arraycopyTexture(FloatBuffer src, int srcPos, FloatBuffer dst,
			int dstPos, int length) {
		if (src == dst) {
			memmoveFloat(src, srcPos * ccQuad2.size, dst,
					dstPos * ccQuad2.size, length * ccQuad2.size);
		} else {
			memcopyFloat(src, srcPos * ccQuad2.size, dst,
					dstPos * ccQuad2.size, length * ccQuad2.size);
		}
	}

	private void arraycopyVertex(FloatBuffer src, int srcPos, FloatBuffer dst,
			int dstPos, int length) {
		if (src == dst) {
			memmoveFloat(src, srcPos * ccQuad3.size, dst,
					dstPos * ccQuad3.size, length * ccQuad3.size);
		} else {
			memcopyFloat(src, srcPos * ccQuad3.size, dst,
					dstPos * ccQuad3.size, length * ccQuad3.size);
		}
	}

	private void arraycopyColor(FloatBuffer src, int srcPos, FloatBuffer dst,
			int dstPos, int length) {
		if (src == dst) {
			memmoveFloat(src, srcPos * ccColor4B.size * 4, dst, dstPos
					* ccColor4B.size * 4, length * ccColor4B.size * 4);
		} else {
			memcopyFloat(src, srcPos * ccColor4B.size * 4, dst, dstPos
					* ccColor4B.size * 4, length * ccColor4B.size * 4);
		}
	}

	private void memmoveFloat(FloatBuffer src, int from, FloatBuffer dst,
			int to, int size) {
		if (to < from) {
	...
	private void memcopyFloat(FloatBuffer src, int from, FloatBuffer dst,
			int to, int size) {
		src.position(from);
		dst.position(to);
		if (BufferUtils.USING_NATIVE) {
	...
	public static void memmoveByte(ByteBuffer src, int from, ByteBuffer dst,
			int to, int size) {
		if (to < from) {
			for (int i = 0; i < size; i++) {
				dst.put(i + to, src.get(i + from));
	...
	public static void memcopyByte(ByteBuffer src, int from, ByteBuffer dst,
			int to, int size) {
		src.position(from);
		dst.position(to);
		if (BufferUtils.USING_NATIVE) {
