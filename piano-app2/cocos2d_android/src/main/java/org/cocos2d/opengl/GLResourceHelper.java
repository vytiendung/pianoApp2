...
/**
 * This class performs tasks on the side of OpenGL thread. CCTexture2D calls
 * perform() in finalize method, and texId is queued to be deleted later.
 * 
 * @author genius
 */
...
public class GLResourceHelper {
	...
	/**
	 * These objects are stored in reloadQueue, they should be removed manually,
	 * DO NOT STORE REFERENCE TO CCTexture2D in TextureLoader, GLResourceLoader
	...
	public GLResourceHelper() {
		...
		reloadMap = Collections
				.synchronizedMap(new WeakHashMap<GLResourceHelper.Resource, GLResourceHelper.GLResourceLoader>());
	}

	public void addLoader(final Resource res, final GLResourceLoader loader,
			boolean addTask) {
		if (addTask) {
		...
		taskQueue.add(new GLResorceTask() {
			...
			public void perform(GL10 gl) {
				synchronized (reloadMap) {
					for (Entry<Resource, GLResourceLoader> entry : reloadMap
							.entrySet()) {
	...
	/**
	 * Add OGL task in queue. If perform is called from another task then task
	...
	private long mTotalTextMemoryUsage = 0;

	public void increamentTextMemory(long mem) {
		synchronized ( reloadMap ) {
			mTotalTextMemoryUsage += mem;
		}
	}

	public long getTextMemoryUsage() {
		return mTotalTextMemoryUsage;
	}
	
	public void removeLoader ( final Resource res ) {
		synchronized (reloadMap) {
			reloadMap.remove(res);
		}
	}
	// end
}
