        ...
        if (_name != 0) {
    		GLResourceHelper.sharedHelper().perform( new GLResourceHelper.GLResorceTask() {
				...
				public void perform(GL10 gl) {
					GLResourceHelper.sharedHelper().removeLoader(CCTexture2D.this);
					if ( mCreator == gl ) {
						GLResourceHelper.sharedHelper().increamentTextMemory( -mTextureSize );
...
//		CCTextureCache.sharedTextureCache().addTexture(this);
		...
		_format = image.getConfig();
		int width = toPow2(mWidth);
		int height = toPow2(mHeight);
		int bytePerPixels = 1;
		if ( _format == Config.ARGB_8888 ) {
			bytePerPixels = 4;
		} else if ( _format == Config.RGB_565 || _format == Config.ARGB_4444 ) {
			bytePerPixels = 2;
		}
		final long newTextureSize = width*height*bytePerPixels;
...
//        }
    ...
    public void loadTexture(GL10 gl) {
        if (_name == 0) {
        	...
        	if(mBitmap == null)
        		return;
        	Log.d("CCTexture2D","loadTexture with : " + mTextureSize + " , " + _name + " , " + (( mTextContent != null ) ? mTextContent : mFilePath ) + " , " + mBitmap.getConfig().toString());
        	GLResourceHelper.sharedHelper().perform( new GLResourceHelper.GLResorceTask() {
            ...
            if ( mFilePath != null || mTextContent != null ) {
	            mBitmap.recycle();
	            mBitmap = null;
            }
            //end
