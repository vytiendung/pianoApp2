...
public class MidiOptions implements Serializable {
    ...
    public float[] velocityRate;     /** Velocity for each track*/
    ...
    public boolean slideToExpand;
    public int bgColor;
    public boolean useDefaultBanks;
    public int[] banks;
    ...
    public MidiOptions(MidiFile midifile) {
        ...
        velocityRate=new float[num_tracks];
...
//            }
        ...
        useDefaultBanks = true;
        banks = new int[num_tracks];
    ...
    public static MidiOptions fromJson(String jsonString) {
        ...
        try {
            ...
            options.freePlay=json.getBoolean("freePlay");
            options.bgColor=json.getInt("bgColor");
        }
        catch (Exception e) {
            return null;
        }
    ...
    public MidiOptions copy() {
        ...
        options.velocityRate = new float[tracks.length];
        for (int i = 0; i < tracks.length; i++) {
            options.tracks[i] = tracks[i];
            options.velocityRate[i]=velocityRate[i];
        }
