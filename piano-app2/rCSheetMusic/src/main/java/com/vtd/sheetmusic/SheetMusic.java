...
import com.vtd.sheetmusic.midi.MidiEvent;
...
import com.vtd.sheetmusic.midi.TimeSignature;
import com.vtd.sheetmusic.symbol.AccidSymbol;
import com.vtd.sheetmusic.symbol.BarSymbol;
import com.vtd.sheetmusic.symbol.BlankSymbol;
import com.vtd.sheetmusic.symbol.ChordSymbol;
import com.vtd.sheetmusic.symbol.Clef;
import com.vtd.sheetmusic.symbol.ClefMeasures;
import com.vtd.sheetmusic.symbol.ClefSymbol;
import com.vtd.sheetmusic.symbol.LyricSymbol;
import com.vtd.sheetmusic.symbol.MusicSymbol;
import com.vtd.sheetmusic.symbol.RestSymbol;
import com.vtd.sheetmusic.symbol.TimeSigSymbol;
import com.vtd.sheetmusic.util.ListInt;
import com.vtd.sheetmusic.util.SymbolWidths;
...
public class SheetMusic extends SurfaceView implements SurfaceHolder.Callback {
	final static String TAG=SheetMusic.class.getSimpleName();
    ...
    private int		averageStaffHeight; 	
    ...
    public SheetMusic(Context context) {
      ...
      //TODO: Giang
        ...
        mProgress = context.getResources().getDrawable(R.drawable.msheet_bass);
    }
...
//          new ArrayList<ArrayList<MusicSymbol> >(numtracks);
    ...
    private void calculateSize() {
        ...
        averageStaffHeight=sheetheight/staffs.size();
        if(staffs.size()>2&&primaryTrack-1>=0){
        	scrollY=averageStaffHeight*(primaryTrack-1);
        }else if(staffs.size()==1){
//        	scrollY=-(viewheight-sheetheight)/2;
        	Log.d(TAG,"-- scrollY="+scrollY);
        }
        checkScrollBounds();
        //Log.d(TAG, "Sheet Width="+sheetwidth+" sheet height="+sheetheight);
    }
    ...
    protected void onMeasure(int widthspec, int heightspec) {
        ...
        if (specwidth == 0 && specheight == 0) {
            ...
            //Log.d(TAG, "1.setMeasuredDimension: "+screenwidth+":"+screenheight);
        }
        else if (specwidth == 0) {
            setMeasuredDimension(screenwidth, specheight);
            //Log.d(TAG, "2.setMeasuredDimension: "+screenwidth+":"+specheight);

        }
        else if (specheight == 0) {
            setMeasuredDimension(specwidth, screenheight);
            //Log.d(TAG, "3.setMeasuredDimension: "+specwidth+":"+screenheight);

        }
        else {
            setMeasuredDimension(specwidth, specheight);
            //Log.d(TAG, "4.setMeasuredDimension: "+specwidth+":"+specheight);
    ...
    onSizeChanged(int newwidth, int newheight, int oldwidth, int oldheight) {
        ...
        Log.d(TAG, "onSizeChanged "+viewwidth+":"+viewheight);
        ...
        if (scrollVert) {
        	...
        	if(staffs.size()>1)
        		zoom=(float)( (newheight + playerHeight)  / (averageStaffHeight*2.0));
        	else
        		zoom=(float)( (newheight + playerHeight)  / (averageStaffHeight*1.4));
        	Log.d(TAG, "=============== zoom = "+zoom + " average="+averageStaffHeight+"  sheet heigh="+sheetheight+"  view height="+newheight);
//            if (zoom < 0.75)
//                zoom = 0.75f;
//            if (zoom > 1.1)
//                zoom = 1.1f;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (!isScrollPositionInBuffer()||zoomDistance>0) {
            drawToBuffer(scrollX, scrollY);
        }
...
//        mProgress.setBounds(getWidth()/2-50, getHeight()-50, getWidth()/2+50,getHeight());
//        mProgress.draw(canvas);
    ...
    private void drawToBuffer(int left, int top) {
        ...
        //Log.d(TAG, " Draw to buffer bX="+bufferX+"  by="+bufferY);
        ...
        paint.setColor(options.bgColor);
    ...
    public void ShadeNotes(int currentPulseTime, int prevPulseTime, 
        ...
        if (!surfaceReady || staffs == null) {
        	Log.d(TAG, "ShadeNotes   surface not ready ");
            return;
        }
        ...
        for (Staff staff : staffs) {
            bufferCanvas.translate(0, ypos);
            if(staffs.indexOf(staff)==primaryTrack||staffs.size()<=1)
	            x_shade = staff.ShadeNotes(bufferCanvas, paint, shade1, 
	                            currentPulseTime, prevPulseTime, x_shade, true);
            else
            	x_shade = staff.ShadeNotes(bufferCanvas, paint, shade2, 
                        currentPulseTime, prevPulseTime, x_shade,false);
         ...
         */
        //Log.d(TAG, " ScrollX="+scrollX+"   bufferX="+bufferX+" bufferX+PageWidth="+(bufferX+PageWidth)+"  scrollY="+scrollY+"  bufferY="+bufferY);
        if (scrollX < bufferX || scrollY < bufferY) {
        	//Log.d(TAG, "Shade note again");
        ...
        SurfaceHolder holder = getHolder();
        Canvas canvas = holder.lockCanvas();
        if (canvas == null || bufferBitmap.isRecycled()||bufferBitmap==null) {
            return;
        }
        canvas.translate(-(scrollX - bufferX), -(scrollY - bufferY));
        canvas.drawBitmap(bufferBitmap, 0, 0, paint);
    ...
    void ScrollToShadedNotes(int x_shade, int y_shade, boolean scrollGradually) {
    	//Log.d(TAG, "ScrollToShadedNotes "+x_shade+":"+y_shade);
        if (scrollVert) {
            ...
            int x_view = scrollX + viewwidth * 20/100;
            int xmax   = scrollX + viewwidth * 45/100;
    ...
    checkScrollBounds() {
        ...
        //if (scrollY < 0) {
            //scrollY = 0;
        //}
        if (Math.abs(scrollY) > Math.abs(scrollheight - viewheight/2)) {
        	if(scrollY>0)
        		scrollY = Math.abs(scrollheight - viewheight/2);
        	else
        		scrollY = Math.abs(scrollheight - viewheight/2)*-1;
        }
        //Log.d(TAG, "----- checkScrollBounds scrollY="+scrollY);
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if(player!=null&&player.playstate==MidiPlayer.playing) return true;
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                if(event.getPointerCount()>1){
                	...
                	inMotion = true;
                	startX=startMotionX = (int)event.getX();
                	startMotionY=(int) event.getY();
                	zoomDistance=0;
                	scrollDirection=0;

                    flagResize = false;
                    /*if(event.getX()>getWidth()-getResources().getDimension(R.dimen.w_sheet_btn_resize_width)
                            &&event.getX()<getWidth()
                            &&event.getY()>getHeight()-getResources().getDimension(R.dimen.w_sheet_btn_resize_height)
                            &&event.getY()<getHeight()
                            ){
                        flagResize = true;
                    }*/
                    if(event.getX()<getResources().getDimension(R.dimen.w_sheet_btn_resize_width)
                            &&event.getX()>0
                            &&event.getY()>getHeight()-getResources().getDimension(R.dimen.w_sheet_btn_resize_height)
                            &&event.getY()<getHeight()
                            ){
            ...
            case MotionEvent.ACTION_MOVE:
            	 if(event.getPointerCount()>1){
                     // todo: duc prevent zoom
            		 /*if(zoomDistance==0){
            			 zoomDistance=(int) Math.sqrt((event.getX(0)-event.getX(1))*(event.getX(0)-event.getX(1))+(event.getY(0)-event.getY(1))*(event.getY(0)-event.getY(1)));

            		 }else{
            			 int newZoomDistance=(int) Math.sqrt((event.getX(0)-event.getX(1))*(event.getX(0)-event.getX(1))+(event.getY(0)-event.getY(1))*(event.getY(0)-event.getY(1)));
            			 if(Math.abs((float)newZoomDistance/zoomDistance-1)>=0.025){
            				 zoom=zoom+(float)newZoomDistance/zoomDistance-1;
            				 zoom=zoom>0.75f?zoom:0.75f;
            				 zoom=zoom<2.5f?zoom:2.5f;
            				 //Log.d(TAG, " Change zoomDistance="+zoomDistance+"   newzoom="+newZoomDistance+" zoom="+zoom );
                     ...
                     if(Math.abs(deltaX)<5&&Math.abs(deltaY)<5) return true;
                     startMotionX = (int)event.getX();
                     startMotionY = (int)event.getY();

						if (options.slideToExpand) {
							//if(Math.abs(deltaY)<30) return true;

                            if(flagResize) {

                                int newHeight = (int) (getHeight() - deltaY);
                                if (newHeight < screenheight / 8)
                                    newHeight = screenheight / 8;
                                if (newHeight < screenheight / 2) {
                                    ...
                                    lp.height = newHeight;
                                    myHeight = newHeight;
                                    listener.changeHeight(myHeight);
                                    this.requestLayout();
                                }
                            }
							return true;
                     ...
                     if (scrollVert) {
                         ...
                         if ((Math.abs(deltaY) > Math.abs(deltaX)) ||
                             (Math.abs(deltaY) > 4)) {
                             // todo: duc prevent horizontal scroll
//                             scrollY += (int)deltaY;
                             // todo: duc end
                         }
                         Log.i(TAG,"duc scroll: "+scrollX);
                     }
                     checkScrollBounds();
                     lastMotionTime = AnimationUtils.currentAnimationTimeMillis();
                     if(!scrollVert&!options.freePlay){
                    	 float a=event.getX()-startX;
                         // duc start: reverse scroll
                         a = -a;
                         // duc end: reverse scroll
                    	 if(player!=null & Math.abs(a)>= NoteWidth*2*zoom&scrollDirection>=0){
                    		 startX=(int)event.getX();
                    		 scrollDirection=1;
                    		 if (a>0)player.FastForwardOneQuater();
                    		 else player.RewindOneQuater();
                    	 }else{
                    		 if(Math.abs(deltaY)>Math.abs(deltaX)&scrollDirection<=0){
                    			 scrollDirection=-1;
                    			 callOnDraw();
                    		 }
                    	 }
	...
	public void setPrimaryTrack(int primaryTrack) {
		this.primaryTrack = primaryTrack;
		if(staffs.size()>2&&primaryTrack-1>=0){
			scrollY=averageStaffHeight*(primaryTrack-1);
		}else if(staffs.size()==1){
        	scrollY=-(viewheight-sheetheight)/2;
        	Log.d(TAG,"-setPrimaryTrack- scrollY="+scrollY);
        }
		checkScrollBounds();
		callOnDraw();
	}
    ...
    public void setBufferPosition(int bufferX, int bufferY){
    ...
    // todo: duc add
    public double getNextPulseTime(double currentPulse){
        double nextPulseTime = 0;
        for(int i = 0;i<allsymbols.get(getPrimaryTrack()).size();i++){

            if(i+1<allsymbols.get(getPrimaryTrack()).size()){
                MusicSymbol ms = allsymbols.get(getPrimaryTrack()).get(i);
                if(ms.getStartTime() == currentPulse){
                    nextPulseTime = allsymbols.get(getPrimaryTrack()).get(i+1).getStartTime();
                }
            }
        }
    ...
    public ArrayList<MidiNote> nextGroup(double currentPulse){
        ArrayList<MidiNote> mGroup = new ArrayList<MidiNote>();
        for(int i=0;i<allsymbols.get(primaryTrack).size();i++){
            MusicSymbol ms = allsymbols.get(primaryTrack).get(i);
            if(ms instanceof ChordSymbol && ms.getStartTime()>=currentPulse){
                for(int j=0;j<((ChordSymbol) ms).getNotedata().length;j++){
                    NoteData note = ((ChordSymbol) ms).getNotedata()[j];
                    MidiNote mNote = new MidiNote(ms.getStartTime(),0,note.number,time.DurationToTime(note.duration));
                    mGroup.add(mNote);
                }
                break;
            }
    ...
    public void scrollSheet(int distanceY) {
//        int newHeight = (int) (getHeight() + distanceY);
        int newHeight = (int) (distanceY);
        if (newHeight < screenheight / 8)
            newHeight = screenheight / 8;
        if (newHeight < screenheight / 2) {
            if (bufferBitmap != null) {
    ...
    public OnSheetLayoutChange listener;
    public void bindOnSheetLayoutChange(OnSheetLayoutChange ls){
        listener = ls;
    }
    public interface OnSheetLayoutChange{
