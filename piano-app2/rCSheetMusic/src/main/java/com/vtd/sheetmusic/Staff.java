...
public class Staff {
    ...
    public int ShadeNotes(Canvas canvas, Paint paint, int shade,
                           int currentPulseTime, int prevPulseTime, int x_shade, boolean flag) {

        boolean flagRedraw = false;
        ...
        for (int i = 0; i < symbols.size(); i++) {
            ...
            if(start<=currentPulseTime&& currentPulseTime<end){
                ...
                canvas.drawRoundRect(new RectF(0, 0, curr.getWidth(), this.getHeight()), 10f, 10f, paint);
                paint.setStyle(Paint.Style.STROKE);
                paint.setColor(Color.BLACK);
                curr.Draw(canvas, paint, ytop);
                canvas.translate(-xpos, 0);
            ...
            if ((start <= currentPulseTime) && (currentPulseTime < end)) {
                ...
                if(flag==true){
                    int n_shape = x_shade+curr.getWidth();
                    // todo: duc add
                    for(int j = i+1;j<symbols.size();j++){
                    /*if(symbols.get(j)instanceof BarSymbol){
                        n_shape = symbols.get(j).getWidth()+n_shape;
                    }else{
                        next = symbols.get(j);
                        break;
                    }*/
                    ...
                    if(next!=null){
                        ...
                        int vtShade = keysigWidth;
                        MusicSymbol vtCurr;
                        for (int k = 0; k < symbols.size(); k++) {
                            vtCurr = symbols.get(k);
                            if(vtCurr instanceof ChordSymbol){
                                canvas.translate(vtShade, 0);
                                paint.setStyle(Paint.Style.FILL);
                                paint.setColor(bgColor);
                                canvas.drawRoundRect(new RectF(0, 0, vtCurr.getWidth(), this.getHeight()), 10f, 10f, paint);
                                paint.setStyle(Paint.Style.STROKE);
                                paint.setColor(Color.BLACK);
//                                vtCurr.Draw(canvas, paint, ytop);
                                canvas.translate(-vtShade, 0);
                            }
                            vtShade += vtCurr.getWidth();
                        ...
                        canvas.translate(n_shape, 0);
                        paint.setStyle(Paint.Style.FILL);
                        paint.setColor(bgColor);
                        canvas.drawRoundRect(new RectF(0, 0, next.getWidth(), this.getHeight()), 10f, 10f, paint);
                        paint.setStyle(Paint.Style.STROKE);
                        paint.setColor(Color.BLACK);
                        next.Draw(canvas, paint, ytop);
                        canvas.translate(-n_shape, 0);

                        canvas.translate(n_shape, 0);
                        paint.setStyle(Paint.Style.FILL);
                        paint.setColor(Color.parseColor("#80ffc900"));
                        canvas.drawRoundRect(new RectF(0, 0, next.getWidth(), this.getHeight()), 10f, 10f, paint);
                        paint.setStyle(Paint.Style.STROKE);
                        paint.setColor(Color.BLACK);
                        next.Draw(canvas, paint, ytop);
            ...
            if (redrawLines) {
                ...
                if(flag==true&&flagRedraw== true){
                    int x_shade2 = xpos;
                    int n_shape2 = x_shade2+curr.getWidth();
                    // todo: duc add
                    for(int j = i+1;j<symbols.size();j++){
                        if(symbols.get(j)instanceof ChordSymbol){
                            next = symbols.get(j);
                            ...
                            n_shape2 = symbols.get(j).getWidth()+n_shape2;
                        }
                    }
                    int line2 = 1;
                    int y2 = ytop - SheetMusic.LineWidth;
                    paint.setStyle(Paint.Style.STROKE);
                    paint.setColor(Color.BLACK);
                    paint.setStrokeWidth(1);
                    canvas.translate(n_shape2-2, 0);
                    for (line2 = 1; line2 <= 5; line2++) {
                        if(next!=null)
                        canvas.drawLine(0, y2, next.getWidth()+4, y2, paint);
                        y2 += SheetMusic.LineWidth + SheetMusic.LineSpace;
                    }
                    canvas.translate(-(n_shape2-2), 0);
                    // todo: duc end

                    // todo: duc redraw note before
                    int vtShade = keysigWidth;
                    MusicSymbol vtCurr;
                    for (int k = 0; k < symbols.size(); k++) {
                        ...
                        if(vtCurr instanceof ChordSymbol){
                            for (line2 = 1; line2 <= 5; line2++) {
                                if(next!=null)
                                    canvas.drawLine(0, y2, vtCurr.getWidth(), y2, paint);
                                y2 += SheetMusic.LineWidth + SheetMusic.LineSpace;
                            }
                        }
