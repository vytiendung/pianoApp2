...
public class MidiFile {
	...
	protected ArrayList<ArrayList<MidiEvent>> allevents; /** The raw MidiEvents, one list per track */
	protected ArrayList<MidiTrack> tracks ;  /** The tracks of the midifile that have notes */
	protected short trackmode;         /** 0 (single track), 1 (simultaneous tracks) 2 (independent tracks) */
	protected TimeSignature timesig;    /** The time signature */
	protected int quarternote;          /** The number of pulses per quarter note */
	protected int totalpulses;          /** The total length of the song, in pulses */
	protected boolean trackPerChannel;  /** True if we've split each channel into a track */
    private byte[] data;
     ...
     */
    protected ArrayList<MidiEvent> ReadTrack(MidiFileReader file) {
    ...
    public ArrayList<ArrayList<MidiEvent>> ApplyOptionsToEvents(MidiOptions options) {
        ...
        float[] velocityRate= new float[num_tracks]; //TODO: Giang
        boolean[] keeptracks = new boolean[num_tracks];
        for (i = 0; i < num_tracks; i++) {
            instruments[i] = 0;
            keeptracks[i] = true;
            velocityRate[i]=1;//TODO:Giang
        }
        for (int tracknum = 0; tracknum < tracks.size(); tracknum++) {
            ...
            velocityRate[realtrack]= options.velocityRate[tracknum];//TODO:Giang
	    ...
	    ArrayList<MidiEvent> tempoEvents = getTempoEvents(options);

        /* Set the tempo at the beginning of each track */
        for (int trackNum = 0; trackNum < newEvents.size(); trackNum++) {
            MidiEvent midiEvent = CreateTempoEvent(tempoEvents.get(0).Tempo);
            newEvents.get(trackNum).add(0, midiEvent);
        }

        /* Change bank track */
        if ( !options.useDefaultBanks ) {
            for (int tracknum = 0; tracknum < newEvents.size(); tracknum++) {
                byte valBank = 0;
                int bank = 0;
                if ( options.banks.length > tracknum ) {
                    bank = options.banks[tracknum];
                } else {
                    bank = options.banks[options.banks.length-1];
                }
                ...
                for ( int index = 0; index < newEvents.get(tracknum).size(); ++index ) {
                    MidiEvent mevent = newEvents.get(tracknum).get(index);
                    if ( isDrum )
                        mevent.Channel = 9;
                    if ( mevent.EventFlag == EventProgramChange ) {
                        MidiEvent newmevent = new MidiEvent();
                        newmevent.EventFlag = EventControlChange;
                        newmevent.HasEventflag = true;
                        newmevent.DeltaTime = 0;
                        newmevent.StartTime = mevent.StartTime;
                        newmevent.ControlNum = 0;
                        newmevent.Channel = mevent.Channel;
                        newmevent.ControlValue = valBank;
                        newEvents.get(tracknum).add(index, newmevent);
                        ++index;
                        if ( !isDrum )
                            break;
                    }
        ...
        for (int trackNum = 0; trackNum < newEvents.size(); trackNum++) {
            for (MidiEvent midiEvent : newEvents.get(trackNum)) {
                int num = midiEvent.Notenumber + options.transpose;
                if (num < 0)
                    num = 0;
                if (num > 127)
                    num = 127;
                midiEvent.Notenumber = (byte)num;
                if (!options.useDefaultInstruments) {
                    midiEvent.Instrument = (byte)instruments[trackNum];
                }
                midiEvent.Tempo = findTempoForMidiEvent(midiEvent.StartTime, tempoEvents);
                
            	//TODO: GIANG apply velocity rate for note
            	midiEvent.Velocity=(byte) (midiEvent.Velocity*velocityRate[trackNum]);
            }
        }
    ...
    public ArrayList<ArrayList<MidiEvent>> ApplyOptionsPerChannel(MidiOptions options) {
        ...
        float[] velocityRate= new float[16]; //TODO: Giang
        ...
        for (int tracknum = 0; tracknum < tracks.size(); tracknum++) {
            ...
            velocityRate[channel]= options.velocityRate[tracknum]; //TODO:Giang
            if (options.tracks[tracknum] == false || options.mute[tracknum] == true) {
        ...
        /* Set the tempo at the beginning of each track */
        for (int trackNum = 0; trackNum < newEvents.size(); trackNum++) {
            MidiEvent mEvent = CreateTempoEvent(tempoEvents.get(0).Tempo);
        ...
        for (int trackNum = 0; trackNum < newEvents.size(); trackNum++) {
            for (MidiEvent midiEvent : newEvents.get(trackNum)) {
                ...
                if (num > 127)
                    num = 127;
                midiEvent.Notenumber = (byte)num;
                if (!keepchannel[midiEvent.Channel]) {
                    midiEvent.Velocity = 0;
                }else{
                	//TODO: GIANG apply velocity rate for note
                	midiEvent.Velocity=(byte) (midiEvent.Velocity*velocityRate[midiEvent.Channel]);
                }
                if (!options.useDefaultInstruments) {
                    midiEvent.Instrument = (byte)instruments[midiEvent.Channel];
                }
                midiEvent.Tempo = findTempoForMidiEvent(midiEvent.StartTime, tempoEvents);
            }
        }
	...
	private ArrayList<MidiEvent> getTempoEvents(MidiOptions options) {
		ArrayList<MidiEvent> tempoEvents;
		if (options instanceof MidiOptionsWithTempoData){
			tempoEvents = ((MidiOptionsWithTempoData) options).tempoEvents;
		}else{
			tempoEvents = new ArrayList<MidiEvent>();
			MidiEvent fakeTempoEvent = CreateTempoEvent(options.tempo);
			tempoEvents.add(fakeTempoEvent);
		}
		return tempoEvents;
	}

	private int findTempoForMidiEvent(int startTime, ArrayList<MidiEvent> tempoEvents) {
		int tempo = tempoEvents.get(0).Tempo;
		for (int i = 1; i <tempoEvents.size(); i++){
			if(tempoEvents.get(i).StartTime > startTime){
				tempo = tempoEvents.get(i - 1).Tempo;
				break;
			}
		}
		return tempo;
    ...
    public ArrayList<MidiTrack> ChangeMidiNotes(MidiOptions options) {
        ...
        //Log.d("MidiFile", "track size="+options.tracks.length);
        for (int track = 0; track < tracks.size(); track++) {
        	//Log.d("MidiFile", "track ="+track+"="+options.tracks[track]);
...
//        }
    ...
    public static MidiTrack CombineToSingleTrack(ArrayList<MidiTrack> tracks)
    {
        ...
        if (tracks.size() == 0) {
            ...
            for (MidiNote note : track.getNotes()) {
            	//TODO Giang: set instrument
            	note.setInstrument(track.getInstrument());
                result.AddNote(note);
            }
            return result;
        }
     ...
     */
    protected static void CheckStartTimes(ArrayList<MidiTrack> tracks) {
        for (MidiTrack track : tracks) {
            int prevtime = -1;
            for (MidiNote note : track.getNotes()) {
     ...
     */
    protected static ArrayList<MidiTrack>
    ...
    //TODO: Giang
	public void setAllevents(ArrayList<ArrayList<MidiEvent>> allevents) {
